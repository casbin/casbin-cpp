...
bool SyncedEnforcer::EnforceExWithMatcher(const std::string& matcher, std::shared_ptr<IEvaluator> evalator, std::vector<std::string>& explain) {
   std::unique_lock<std::shared_mutex> lock(policyMutex);
   return Enforcer::EnforceExWithMatcher(matcher, evalator, explain);
}
bool SyncedEnforcer::EnforceExWithMatcher(const std::string& matcher, const DataList& params, std::vector<std::string>& explain) {
   std::unique_lock<std::shared_mutex> lock(policyMutex);
   return Enforcer::EnforceExWithMatcher(matcher, params, explain);
}
bool SyncedEnforcer::EnforceExWithMatcher(const std::string& matcher, const DataVector& params, std::vector<std::string>& explain) {
   std::unique_lock<std::shared_mutex> lock(policyMutex);
   return Enforcer::EnforceExWithMatcher(matcher, params, explain);
}
bool SyncedEnforcer::EnforceExWithMatcher(const std::string& matcher, const DataMap& params, std::vector<std::string>& explain) {
   std::unique_lock<std::shared_mutex> lock(policyMutex);
   return Enforcer::EnforceExWithMatcher(matcher, params, explain);
}
// EnforceEx explain enforcement by informing matched rules.
bool SyncedEnforcer::EnforceEx(std::shared_ptr<IEvaluator> evalator, std::vector<std::string>& explain) {
   std::unique_lock<std::shared_mutex> lock(policyMutex);
   return Enforcer::EnforceEx(evalator, explain);
}
bool SyncedEnforcer::EnforceEx(const DataList& params, std::vector<std::string>& explain) {
   std::unique_lock<std::shared_mutex> lock(policyMutex);
   return Enforcer::EnforceEx(params, explain);
}
bool SyncedEnforcer::EnforceEx(const DataVector& params, std::vector<std::string>& explain) {
   std::unique_lock<std::shared_mutex> lock(policyMutex);
   return Enforcer::EnforceEx(params, explain);
}
bool SyncedEnforcer::EnforceEx(const DataMap& params, std::vector<std::string>& explain) {
   std::unique_lock<std::shared_mutex> lock(policyMutex);
   return Enforcer::EnforceEx(params, explain);
}
// EnforceWithMatcher use a custom matcher to decides whether a "subject" can access a "object" with the operation "action", input parameters are usually: (matcher, sub, obj, act), use model matcher by default when matcher is "".
bool SyncedEnforcer::EnforceWithMatcher(const std::string& matcher, std::shared_ptr<IEvaluator> evalator) {
   std::unique_lock<std::shared_mutex> lock(policyMutex);
   return Enforcer::EnforceWithMatcher(matcher, evalator);
}
bool SyncedEnforcer::EnforceWithMatcher(const std::string& matcher, const DataList& params) {
   std::unique_lock<std::shared_mutex> lock(policyMutex);
   return Enforcer::EnforceWithMatcher(matcher, params);
}
bool SyncedEnforcer::EnforceWithMatcher(const std::string& matcher, const DataVector& params) {
   std::unique_lock<std::shared_mutex> lock(policyMutex);
   return Enforcer::EnforceWithMatcher(matcher, params);
}
bool SyncedEnforcer::EnforceWithMatcher(const std::string& matcher, const DataMap& params) {
   std::unique_lock<std::shared_mutex> lock(policyMutex);
   return Enforcer::EnforceWithMatcher(matcher, params);
}
...
// BuildRoleLinks manually rebuild the role inheritance relations.
void SyncedEnforcer::BuildRoleLinks() {
   std::lock_guard<std::shared_mutex> lock(policyMutex);
   Enforcer::BuildRoleLinks();
}
