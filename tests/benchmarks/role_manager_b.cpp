...
static void BenchmarkRoleManagerSmall(benchmark::State& state) {
    ...
    for (int i = 0; i < 100; ++i) params = {"group" + std::to_string(i), "data" + std::to_string(i / 10), "read"}, e.AddPolicy(params);

    // 1000 users.
    for (int i = 0; i < 1000; ++i) g_params = {"user" + std::to_string(i), "group" + std::to_string(i / 10)}, e.AddGroupingPolicy(g_params);

    auto rm = e.GetRoleManager();

    for (auto _ : state) {
        for (int j = 0; j < 100; ++j) rm->HasLink("user501", "group" + std::to_string(j));
    }
}
